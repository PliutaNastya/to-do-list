...
const renderTaskItem = (taskText, taskId) => {
	return `
	<li class="task neumorphic" data-id="${taskId}">
		<span>${taskText}</span>
...
const addNewTask = (taskText) => {
	...
	tasksListElement.insertAdjacentHTML('beforeend', taskItem)
}
...
document.addEventListener('click', (e) => {
	...
	if (targetElement.tagName === 'BUTTON' && targetElement.classList.contains('delete-button')) {
		...
		const currentTaskId = parseInt(currentTaskItem.dataset.id)
		deleteTask(currentTaskId, tasksList)
